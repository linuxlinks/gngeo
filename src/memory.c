...
void dump_hardware_reg(void) {
	...
	if (video_dump == NULL)
		video_dump = SDL_CreateRGBSurface(SDL_SWSURFACE, 512 + 32, 512 + 0x3f
				* 16 + 32, 16, 0xF800, 0x7E0, 0x1F, 0);
	...
	for (i = 0; i < 0x300; i += 2) {
		...
		if (tctl1 & 0x40) {
			...
			if (i != 0) { /* print info of the last plane */
				if (nb_tile > 0) {
					...
					draw_border(video_dump, first_x + 16, y + 16, total_z, ((zy
							+ 1) * nb_tile / 16.0), 0xFE00);
				}
			}
		...
		if (nb_tile > 0) {
			...
			for (j = 0; j < nb_tile; j++) {
				int tileno, tileatr;
				tileno = READ_WORD_ROM(&vidram[offs]);
				offs += 2;
				tileatr = READ_WORD_ROM(&vidram[offs]);
				offs += 2;
				...
				if (tileno <= memory.nb_of_tiles) {
					...
					if (memory.pen_usage[tileno] != TILE_INVISIBLE) {
						//printf("Tile %d %d %d\n",tileno,x,yy);
						debug_draw_tile(tileno, x + 16, yy + 16, 16/*zx+1*/,
								16/*(zy+1)/16.0*/, tileatr >> 8,
