...
@ little customisation for gngeo

.equ mmsp2_regs, (0xc0000000-0x02000000) @ assume we live @ 0x2000000 bank
.equ shared_ctl,  0x01C80000             @ this is where shared_ctl struncture is located
...
.Begin:
    ...
    @ set up region 1 which is the first 32 megabytes.
    mov r0, #(0x18<<1)|1        @ region data
    mcr p15, 0, r0, c6, c1, 0
    mcr p15, 0, r0, c6, c1, 1

    @ set up region 2: mmsp2 regs
    mov r0, #(0x0f<<1)|1
    orr r0, r0, #mmsp2_regs
    mcr p15, 0, r0, c6, c2, 0
    mcr p15, 0, r0, c6, c2, 1

    @ set up region 3: 4M 0x1C00000-0x2000000 (non cached shared data/ctl)
    mov r0, #(0x15<<1)|1
    orr r0, r0, #0x1C00000
    mcr p15, 0, r0, c6, c3, 0
    mcr p15, 0, r0, c6, c3, 1
	
    @ set up region 4: 4M 0x1800000-0x1C00000 (cached shared data/ctl)
    mov r0, #(0x15<<1)|1
    orr r0, r0, #0x1800000
    ...
    @ set up region 4: 16M 0x0100000-0x01100000 (adpcm a/b data area)
    @mov r0, #(0x17<<1)|1
    @orr r0, r0, #0x0100000
    @mcr p15, 0, r0, c6, c4, 0
    @mcr p15, 0, r0, c6, c4, 1
